...
uint8_t waveContinuePlaying(waveHeader_t* wavefile)
{
    ...
    //f_read(0, bytesToPlay, &bytesWritten);
...
uint32_t waveParseHeader(waveHeader_t* wavefile, uint8_t* filename)
{
    ...
    if (f_open(filename) != FR_OK) return WAVE_IO_ERROR + 1;
    if (f_lseek(0) != FR_OK) return WAVE_IO_ERROR + 2;

    /* Check RIFF-WAVE file header */
    if (f_read(Buff, 12, &bytesRead)) return WAVE_IO_ERROR + 3;
    ...
    for (;;)
    {
        ...
        if (f_read(Buff, 8, &bytesRead)) return WAVE_IO_ERROR + 4;
        ...
        switch (LD_DWORD(Buff))
        {
            case FCC('f', 'm', 't', ' '): /* 'fmt ' chunk */
                ...
                if (f_read(Buff, chunkSize, &bytesRead)) return WAVE_IO_ERROR + 5; /* Get the chunk content */
            ...
            case FCC('L', 'I', 'S', 'T'): /* 'LIST' chunk (skip) */
                if (chunkSize & 1) chunkSize++;
                if (f_lseek(filesys.fptr + chunkSize)) return WAVE_IO_ERROR + 6;
                break;
