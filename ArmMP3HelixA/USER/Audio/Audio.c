...
int FillBuffer(FIL *FileObject, uint8_t start)
{
	...
	if (outOfData == 1)
	{
		printf("Playback out of Data! Stopping \n");
		/* Stop Playback */
		AUDIO_Playback_Stop();
		return 0;
	}
...
void PlayAudioFile(FIL *FileObject, char *path)
{
	...
	if ((strstr(path, "WAV") != NULL) || (strstr(path, "wav") != NULL))
	{
		...
		while( SemaphoreTake(Sem_StopMP3Decode, 0) == pdFAIL)
		{
			/* Start the playback */
...
void DMA_Configuration(s8 * addr, int size)
{
	...
	RCC_AHBPeriphClockCmd(DAC_DMA_CLOCK, ENABLE);
	...
	//DMA_ITConfig(DAC_DMA_CH, DMA_IT_TC, ENABLE);
...
void I2S_Configuration(uint32_t I2S_AudioFreq)
{
	...
	RCC_APB1PeriphClockCmd(AUDIO_OUTPUT_TIMER_CLOCK, ENABLE);
	...
	TIM_TimeBaseInit(AUDIO_OUTPUT_TIMER, &TIM_TimeBaseStructure);

	/* TIM2 TRGO selection */
	TIM_SetAutoreload(AUDIO_OUTPUT_TIMER, (uint32_t)(SystemCoreClock / I2S_AudioFreq));
	TIM_SelectOutputTrigger(AUDIO_OUTPUT_TIMER, TIM_TRGOSource_Update);
	TIM_ITConfig(AUDIO_OUTPUT_TIMER, TIM_IT_Update, ENABLE);

	DAC_DeInit();
	DAC_StructInit(&DAC_InitStructure);
	/* DAC channel1 Configuration */
	DAC_InitStructure.DAC_Trigger = DAC_TRGO;
	...
	TIM_Cmd(AUDIO_OUTPUT_TIMER, ENABLE);
}
...
//	OS_EXIT_CRITICAL()	;
	...
	TIM_ClearITPendingBit(AUDIO_OUTPUT_TIMER, TIM_IT_Update);
	TimeOut = 1;
...
//	NVIC_InitStructure.NVIC_IRQChannel = AUDIO_OUTPUT_TIMER_IRQn;
		...
		AUDIO_Format_enum format)
{
	...
	{
		strcpy(mp3_info.title, "Title too large");
	}
