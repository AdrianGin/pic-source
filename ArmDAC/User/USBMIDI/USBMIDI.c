...
uint8_t LookupMIDIMessage(uint8_t MIDIStatusCode)
{
   ...
   {
      for( i = SYS_COMMON_MSG; (MIDILookupTable[i]) ; i = i + 3 )
      {
         if( MIDIStatusCode == (MIDILookupTable[i]) )
         {
            return i;
         }
      }
   ...
   else
   {
      ...
      for( i = 0; (MIDILookupTable[i]) ; i = i + 3 )
      {
         if( MIDIStatusCode == (MIDILookupTable[i]) )
         {
            return i;
...
{
	 uint8_t CIN = (MIDILookupTable[messageIndex+2]);
...
{
	 uint8_t bytesToReturn = (MIDILookupTable[messageIndex+1]);
...
uint8_t MIDIDataReady(uint8_t inByte, usbMIDIcable_t* usbMIDIcable)
{
   ...
   uint8_t messageIndex = 0;
			 ...
			 //If we have a realtime message, return it immediately. Redundant
// 		 if(inByte >= MIDI_TIMING_CLOCK)
// 		 {
// 			 MIDIMessage = &usbMIDIcable->rtMsg;
//       /* Clear */
// 			MIDIMessage->header = 0x0F;
// 			MIDIMessage->MIDIData[0] = inByte;
// 			 
//       MIDIMessage->MIDIData[1] = 0;
//       MIDIMessage->MIDIData[2] = 0;

// 			return MIDI_RTDATA_READY;
// 		 }
// 	
   ...
   /* A new status byte is received, discard the old one,  we can mix RT with any message
      but a NOTE_ON in the middle of a SYSEX is not allowed.	 */
