...
)
{
	...
	bsect = 0;
	fmt = check_fs(&buf[0], bsect);			/* Check sector 0 as an SFD format */

	BYTE outputString[20];
	//outputString[0] = buf[0];
	//outputString[1] = buf[1];
	//outputString[2] = 0;
	uartTx(&PrimaryUART, buf[0]);
	uartTx(&PrimaryUART, buf[1]);
