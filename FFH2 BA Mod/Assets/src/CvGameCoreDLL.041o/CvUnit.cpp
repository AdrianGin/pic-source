                        ...
                        if (pLoopUnit->isEnemy(getTeam()))
                        {
                            if (!pLoopUnit->isImmuneToFear())
                            {
                                if (GC.getGameINLINE().getSorenRandNum(20, "Im afeared!") <= (baseCombatStr() + 10 - pLoopUnit->baseCombatStr()))
                                {
                                    ...
                                    pLoopUnit->withdrawlToNearestValidPlot(false);
                                }
                            }
                        }
                    }
...
//FfH: Modified by Kael 06/20/2010
...
//FfH: Added by Kael 06/22/2010
bool CvUnit::isInvisibleInPlot(TeamTypes eTeam, const CvPlot* pPlot, bool bDebug, bool bCheckCargo) const
{
	if (bDebug && GC.getGameINLINE().isDebugMode())
	{
		return false;
...
//								pLoopUnit->jumpToNearestValidPlot(); // can kill unit
                                ...
                                {
                                    if (!isInvisibleInPlot(pLoopUnit->getTeam(), pNewPlot, false))
                                    {
                                        pLoopUnit->withdrawlToNearestValidPlot(true); // can kill unit
                                    }
                                }
//FfH: End Modify
...
void CvUnit::changeInvisibleFromPromotion(int iNewValue)
{
    if (iNewValue != 0)
    {
        ...
        if (!isInvisibleFromPromotion() && plot()->isVisibleEnemyUnit(this))
        {
            ...
            {
                withdrawlToNearestValidPlot(true);
            }
        }
    }
}
...
{
    if (isDelayedDeath()) //prevents some pyre zombie caused CtD's 05/23/2010
    {
        return;
    }
    CvWString szMessage;
...
bool CvUnit::withdrawlToNearestValidPlot(bool bKill)
{
	CvPlot* pLoopPlot;
	CvPlot* pBestPlot = NULL;
	int iValue;
