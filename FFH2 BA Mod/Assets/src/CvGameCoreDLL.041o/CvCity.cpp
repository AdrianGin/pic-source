...
//FfH: Added by Kael 04/11/2008
    ...
    {
        if (GC.getProjectInfo(eProject).getPrereqCivilization() != getCivilizationType())
        {
            return false;
        }
    }
...
**/
            int neededturn= GC.getBuildingInfo(eBuilding).getCommerceChangeDoubleTime(eIndex)*GC.getGameSpeedInfo(GC.getGameINLINE().getGameSpeedType()).getConstructPercent()/100;
